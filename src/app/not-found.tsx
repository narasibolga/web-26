...
function detectLocale(accept: string | null): "id" | "en" {
  if (!accept) return "id";
  const prefs = accept
    .split(",")
  ...
  for (const pref of prefs) {
    if (pref === "id" || pref.startsWith("id-")) return "id";
...
export default async function NotFound() {
  ...
  return (
    <>
      ...
      `}</style>
      <main
        className={`relative flex min-h-dvh flex-col items-center justify-center bg-background px-6 py-24 text-center font-sans text-foreground ${notoSerif.variable} ${brandon.variable}`}
        lang={t.html}
      >
